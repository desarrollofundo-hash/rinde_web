                                ...
                                {revisiones.map((revision, index) => (
                                    <article key={index} className="rounded-xl border border-slate-200 bg-slate-50/50 p-4">
                                        <div className="mb-3 flex items-start justify-between gap-3">
                                            <h3 className="text-sm font-bold text-slate-800">{revision?.titulo ?? revision?.title ?? "Sin título"}</h3>
                                            <span className={`shrink-0 rounded-full px-2 py-1 text-[11px] font-semibold ${getEstadoBadgeClass(revision)}`}>
                                                {getEstadoLabel(revision)}
                                            </span>
                                        </div>
                ...
                {selectedRevision && (
                    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 sm:items-center sm:p-4">
                        ...
                        <div className="relative z-10 flex w-full flex-col rounded-t-3xl border border-slate-200 bg-white shadow-2xl sm:max-w-2xl sm:rounded-2xl" style={{ maxHeight: "90dvh" }}>
                            {/* Header */}
                            <div className="sticky top-0 rounded-t-3xl border-b border-slate-200 bg-linear-to-r from-slate-100 to-cyan-50 sm:rounded-t-2xl">
                                {/* Título + botón cerrar */}
                                <div className="flex items-start justify-between gap-3 px-4 pt-4 pb-3 sm:px-6">
                                    ...
                                    <button
                                        onClick={handleCerrarDetalles}
                                        className="shrink-0 flex h-8 w-8 items-center justify-center rounded-full text-slate-400 transition hover:bg-slate-200 hover:text-slate-700 cursor-pointer"
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                                    </button>
                                </div>
                                        ...
                                        <div className="hidden overflow-x-auto sm:block">
                                            <table className="w-full text-sm">
                                                <thead className="border-b border-slate-200 bg-slate-50">
                                                    <tr>
                                                        <th className="px-4 py-3 text-left font-semibold text-slate-700">ID Gasto</th>
                                                ...
                                                <tbody>
                                                    {detalles.map((detalle, idx) => (
                                                        <tr key={idx} className="border-t border-slate-100 hover:bg-slate-50/70">
                                                            <td className="px-4 py-3 text-slate-700">{getDetalleRendId(detalle) || "-"}</td>
                                                            <td className="px-4 py-3 text-slate-700">{getDetalleProveedor(detalle)}</td>
                                                            <td className="px-4 py-3 text-slate-700">{getDetalleCategoria(detalle)}</td>
                                                            <td className="px-4 py-3 text-slate-700">{getDetalleFecha(detalle)}</td>
                                                            <td className="px-4 py-3 text-right font-medium text-slate-800">S/ {getDetalleMonto(detalle).toFixed(2)}</td>
                                                            <td className="px-4 py-3">
                                                                <button
                                                                    type="button"
                                                                    className="inline-flex items-center gap-1 rounded-md bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700 border border-emerald-200 hover:bg-emerald-100 transition cursor-pointer"
                                            ...
                                            {detalles.map((detalle, idx) => (
                                                <div key={idx} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                                    <div className="flex items-center justify-between gap-2">
                                                        <p className="min-w-0 flex-1 truncate text-sm font-semibold text-slate-800">{getDetalleProveedor(detalle)}</p>
                                                        <div className="flex shrink-0 items-center gap-1.5">
                                                            ...
                                                            <button
                                                                type="button"
                                                                className="inline-flex items-center justify-center rounded-lg bg-emerald-50 p-1.5 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 transition"
                                                                onClick={() => setDetalleRevision(detalle)}
                                                            >
                                                    ...
                                                    <div className="mt-1.5 grid grid-cols-2 gap-x-3 gap-y-0.5 text-xs text-slate-600">
                                                        <p><span className="font-medium text-slate-700">ID:</span> {getDetalleRendId(detalle) || "-"}</p>
                                                        <p><span className="font-medium text-slate-700">Fecha:</span> {getDetalleFecha(detalle)}</p>
                                                        <p className="col-span-2"><span className="font-medium text-slate-700">Categoría:</span> {getDetalleCategoria(detalle)}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
